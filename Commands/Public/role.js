...
module.exports = (bot, db, config, winston, userDocument, serverDocument, channelDocument, memberDocument, msg, suffix, commandData) => {
	if(suffix) {
		if(suffix.indexOf("|")>-1) {
			...
			if(args[0]) {
				var role = bot.roleSearch(args[0], msg.guild);
				if(role && role.id!=msg.guild.id) {
					if(msg.member.permission.has("manageRoles")) {
						if(!args[1] || args[1]==".") {
							...
							var member = bot.memberSearch(args[1], msg.guild);
							if(member) {
			...
			var role = bot.roleSearch(suffix, msg.guild);
			if(role) {
		...
		var info = msg.member.roles.map(roleid => {
