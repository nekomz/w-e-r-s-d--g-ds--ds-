...
module.exports = (bot, db, config, winston, userDocument, serverDocument, channelDocument, memberDocument, msg, suffix, commandData) => {
	if(suffix) {
		switch(suffix.toLowerCase()) {
			...
			case "enroll":
				if(channelDocument.lottery.isOngoing) {
					var ticketPrice = Math.floor(channelDocument.lottery.participant_ids.length * Lotteries.multiplier);
					if(userDocument.points>=ticketPrice) {
						var userTicketCount = channelDocument.lottery.participant_ids.reduce((n, usrid) => {
			...
			case ".":
				if(channelDocument.lottery.isOngoing) {
					if(channelDocument.lottery.creator_id==msg.author.id || bot.getUserBotAdmin(msg.guild, serverDocument, msg.member)>=1) {
						var winner = Lotteries.end(db, msg.guild, serverDocument, msg.channel, channelDocument);
						if(!winner) {
							ch.createMessage("Lottery ended. No one won, though ");
						}
					}
				} else {
					showNotStarted();
		...
		if(channelDocument.lottery.isOngoing) {
			var creator = msg.guild.members.get(channelDocument.lottery.creator_id);
			var participantCount = channelDocument.lottery.participant_ids.filter((elem, i, self) => {
			    return i==self.indexOf(elem);
			}).length;
