	...
	if(suffix) {
		if(suffix.indexOf("|")>-1) {
			...
			if(args.length==2 && args[0].trim() && (!args[1].trim() || [".", "+1", "-1"].indexOf(args[1].trim())>-1)) {
				var countDocument = serverDocument.config.count_data.id(args[0].toLowerCase().trim());
				if(countDocument) {
					switch(args[1].trim()) {
						case "":
						case ".":
			...
			var countDocument = serverDocument.config.count_data.id(suffix.toLowerCase());
			if(countDocument) {
		...
		var info = serverDocument.config.count_data.map(countDocument => {
