...
module.exports = (bot, db, config, winston, userDocument, serverDocument, channelDocument, memberDocument, msg, suffix, commandData) => {
	if(suffix) {
		if(suffix.indexOf("|")>-1) {
				...
				findRoleOrMember(args[0].trim(), (target, type) => {
					if(target && type) {
						...
						if(PermissionConstants.hasOwnProperty(perm)) {
							...
							function doOverwrite(allow, deny) {
			...
			findRoleOrMember(suffix, (target, type) => {
				if(target && type) {
					var overwrite = msg.channel.permissionOverwrites.find(a => {
						return a.id==target.id && a.type==type;
					});
					var perms = type=="role" ? target.permissions.json : msg.channel.permissionsOf(target.id).json;
					if(overwrite) {
						for(var perm in overwrite.json) {
	...
	function findRoleOrMember(str, callback) {
		...
		if(role) {
			...
			var member = bot.memberSearch(str, msg.guild);
			if(member) {
