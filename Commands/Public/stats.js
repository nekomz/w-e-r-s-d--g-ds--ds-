...
module.exports = (bot, db, config, winston, userDocument, serverDocument, channelDocument, memberDocument, msg, suffix) => {
	if(suffix=="clear") {
		...
		var mostPlayedGames = serverDocument.games.sort((a, b) => {
		...
		var mostUsedCommands = serverDocument.command_usage ? Object.keys(serverDocument.command_usage).map(a => {
			return {
				_id: a,
		...
		db.users.find({
			...
			var richestMembers = userDocuments ? userDocuments.map(a => {
