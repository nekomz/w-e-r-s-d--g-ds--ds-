module.exports = (bot, db, config, winston, userDocument, msg, suffix, commandData) => {
	if(suffix) {
		if(suffix.indexOf("|")>-1) {
			var nick = suffix.substring(0, suffix.indexOf("|")).trim();
			var svrname = suffix.substring(suffix.indexOf("|")+1).trim();
			if(nick) {
		        var serverNickDocument = userDocument.server_nicks.id(nick);
		        if(serverNickDocument) {
		        	if(!svrname || svrname==".") {
		        		...
		        		var svr = bot.guilds.get(serverNickDocument.server_id);
		        		if(svr) {
		        			var targetSvr = bot.serverSearch(svrname, msg.author, userDocument);
		        			if(targetSvr) {
		        	...
		        	var svr = bot.serverSearch(svrname, msg.author, userDocument);
		        	if(svr) {
        	...
        	var serverNickDocument = userDocument.server_nicks.id(suffix.toLowerCase());
        	if(serverNickDocument) {
        		var svr = bot.guilds.get(serverNickDocument.server_id);
        ...
        var info = userDocument.server_nicks.filter(serverNickDocument => {
