...
module.exports = (bot, db, winston, svr, serverDocument, ch, extensionDocument, msg, suffix, keywordMatch) => {
	var extensionSandbox = new vm.createContext(getSandbox(bot, db, winston, extensionDocument, svr, serverDocument, ch, msg, suffix, keywordMatch));
	fs.readFile(__dirname + "/../Extensions/" + svr.id + "-" + extensionDocument._id + ".abext", "utf8", (err, extensionCode) => {
		if(err) {
			...
			try {
			    ...
			    extensionDomain.run(() => {
					var extensionVM = new vm.Script(extensionCode);
